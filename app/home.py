"""
Home page of the BioData-QC app (the main Streamlit view).

Registered as a page by the entrypoint in ``main.py``; it renders the whole
application except the standalone About page.
"""
...
# Brand-icon links for the top bar. `st.link_button` only supports
# Material/emoji icons, so these are plain anchors with inline SVG logos.
# They are squared with rounded corners so they match the "About" button.
SOCIAL_LINKS_STYLE = """
<style>
.social-links { display: inline-flex; align-items: center; gap: 0.5rem; flex-wrap: nowrap; line-height: 0; }
.social-links a {
...
.social-links a:hover {
    ...
    box-shadow: 0 6px 16px color-mix(in srgb, var(--primary-color, #FF4B4B) 40%, transparent);
}
.social-links a svg { width: 1.15rem; height: 1.15rem; fill: currentColor; }
.social-links .topbar-action {
    display: inline-flex; align-items: center; justify-content: center;
    width: auto; height: 2.5rem; padding: 0 0.9rem; border-radius: 0.5rem;
    background: transparent;
    border: 1px solid color-mix(in srgb, currentColor 30%, transparent);
    color: inherit; text-decoration: none;
    font-size: 0.875rem; font-weight: 500; line-height: 1.25; white-space: nowrap;
    transition: background 0.15s ease, color 0.15s ease, border-color 0.15s ease,
                transform 0.15s ease, box-shadow 0.15s ease;
}
.social-links .topbar-action:hover {
    background: var(--primary-color, #FF4B4B);
    border-color: var(--primary-color, #FF4B4B);
    color: #ffffff;
    transform: translateY(-1px);
...
<div class="social-links">
  ...
  <a class="topbar-action" href="./about" target="_blank" rel="noopener noreferrer">About the Author</a>
</div>
"""

# Styling for the top bar (brand + action buttons).
TOPBAR_STYLE = """
<style>
.st-key-topbar {
    background: color-mix(in srgb, var(--secondary-background-color, #808080) 30%, transparent) !important;
    border: 1px solid color-mix(in srgb, currentColor 18%, transparent) !important;
    border-radius: 0.85rem !important;
    box-shadow: 0 1px 2px rgba(0, 0, 0, 0.05), 0 10px 30px rgba(0, 0, 0, 0.12);
    padding: 0.4rem 0.65rem !important;
}
.st-key-topbar [data-testid="stMarkdown"] { margin: 0 !important; }
.st-key-topbar [data-testid="stMarkdownContainer"] {
    display: flex; align-items: center; margin: 0 !important;
}
/* Push the brand to the far left and keep the actions on the right. */
.st-key-topbar_brand { margin-right: auto; }
.st-key-topbar_brand [data-testid="stVerticalBlock"] { gap: 0 !important; }
.topbar-brand {
...
.topbar-brand__mark {
    ...
    background: var(--primary-color, #FF4B4B); color: #ffffff; font-size: 1rem;
}
.topbar-brand__muted { opacity: 0.6; font-weight: 600; }
</style>
"""
...
<div class="topbar-brand">
  ...
  <span>BioData<span class="topbar-brand__muted">-QC</span></span>
</div>
"""
...
# CSS for the first-page upload control
...
# Config
st.set_page_config(page_title="BioData QC & 3D Viewer", page_icon="🧬", layout="wide")
    ...
    with st.container(border=True):
        ...
        st.markdown(r"""
            ...
            **Important - Expected Column Meanings**
            ...
            3. **Wild-Type Affinity Column**: Equilibrium dissociation constant ($K_d$) of the wild-type protein (M).
            4. **Mutant Affinity Column**: Equilibrium dissociation constant ($K_d$) of the mutant protein (M).
        ...
        with col_btn2:
            # The uploader IS the action: clicking it opens the file picker directly.
...
# Placeholder kept above the "Data loaded" message. It is filled with the top
# bar further down, but only once the pipeline results (dashboard) exist.
top_bar_ph = st.empty()
...
with st.sidebar:
    ...
    # Instructions available at any time
...
with st.sidebar.form(key="pipeline_config_form"):
    ...
    col_aff_wt = st.selectbox("Wild-Type Affinity Column (Numeric Float):", options=column_options,
                                ...
                                help="Equilibrium dissociation constant ($K_d$) of the wild-type protein (M).")
    col_aff_mut = st.selectbox("Mutant Affinity Column (Numeric Float):", options=column_options,
                                ...
                                help="Equilibrium dissociation constant ($K_d$) of the mutant protein (M).")
    ...
    z_thresh = st.slider("Robust Z-Score Cutoff:", 1.5, 5.0, DEFAULT_QC_PARAMS["z_thresh"], 0.1, 
                            help="Threshold for the robust Z-score outlier detection. Data points with |Z| above this value are flagged as outliers. Robust Z-score is calculated by grouping the data by PDB ID and using the median and MAD (Median Absolute Deviation) to reduce the influence of extreme values.")
    iso_contam = st.slider("Isolation Forest Contamination:", 0.01, 0.15, DEFAULT_QC_PARAMS["iso_contam"], 0.01,
                            help="Estimated proportion of outliers in the dataset for the Isolation Forest algorithm. A higher value makes the model more sensitive to outliers, while a lower value makes it more conservative.")
...
if submit_button:
    # Validate that every required column has been selected (selectboxes start empty for uploaded CSVs, so a None here means the user skipped one).
...
if 'df_qc' in st.session_state:
    ...
    # --- TOP BAR: brand + contact links + author page link ---
    with top_bar_ph:
        with st.container(horizontal=True, horizontal_alignment="right", vertical_alignment="center",
                          border=True, key="topbar"):
            with st.container(key="topbar_brand"):
                st.markdown(TOPBAR_STYLE + TOPBAR_BRAND_HTML, unsafe_allow_html=True)
    ...
    with tab1:
        ...
        with dl_col1:
            st.download_button(
                ...
                file_name=f"original_dataset_{pd.Timestamp.now().strftime('%Y-%m-%d_%H-%M-%S')}.csv",
                mime="text/csv",
                width='stretch',
            )
        with dl_col2:
            ...
            st.download_button(
                ...
                file_name=f"processed_dataset_{pd.Timestamp.now().strftime('%Y-%m-%d_%H-%M-%S')}.csv",
                mime="text/csv",
                width='stretch',
            )
        ...
        if available_pdbs:
            ...
            with controls_col:
                ...
                with opt_col1:
                    ...
                    struct_opacity = st.slider(
                        "Structure opacity:",
                        min_value=10, max_value=100, value=100, step=5,
                        key="tab2_opacity",
                        help="Transparency of the main representation. At 100% the structure is solid (no pale/ghosted look); lower values make it see-through.",
                    )
            ...
            with viewer_col:
                ...
                success_wt, wt_data = render_wt_structure_highlight(
                    ...
                    mut_repr=mut_repr,
                    bg_color=bg_color,
                    struct_opacity=struct_opacity / 100,
                ...
                if report and report.get("key") == report_key:
                    ...
                    st.download_button(
                        ...
                        file_name=f"{selected_pdb}_{safe_mut}_executive_summary_{pd.Timestamp.now().strftime('%Y-%m-%d_%H-%M-%S')}.html",
                        mime="text/html",
                        key="dl_report",
                    )
                else:
